...
class MyTree{
	...
	public boolean isBST(){
		return isBSTCheckUsingRange(root, -254646464, 232564648);
	}

	public boolean isBSTCheck(Node root){
		if(root == null)
			return true;
		if(isLeafNode(root))
			return true;

		if(isBSTCheck(root.left) && isBSTCheck(root.right)){
			boolean check = false;
	...
	// method does not work for duplicates as when min/max is updated we do +/-1 to the data which makes the duplicate be out of range in the next comparison
	public boolean isBSTCheckUsingRange(Node root, int min, int max){
		if(root == null)
			return true;
		if(root.data < min || root.data > max)
			return false;

		System.out.println(root.data + " " + min + " " + max);

		return (isBSTCheckUsingRange(root.left, min, root.data-1) && isBSTCheckUsingRange(root.right, root.data+1, max));
	}
}
