...
class MyTree{
	...
	public createBST(int[] arr){
		root = createBSTFromArray(arr,0,arr.length-1);
	}

	public Node createBSTFromArray(int[] arr, int low, int high){
		if( low > high )
			return null;
		...
		temp.left = createBSTFromArray(arr, low, mid-1);
		temp.right = createBSTFromArray(arr, mid+1, high);

		return temp;
	}
}

public class createBSTfromSArr{
	static MyTree bst;
	...
	public static void main(String args[]){
		createBSTfromSArr bc = new createBSTfromSArr();
		int[] arr = {2,5,6,8,9,10,12,15,16,18,20};
		bst.createBST(arr);
		bst.inOrder();
	}
}